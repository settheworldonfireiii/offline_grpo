...
def test_log_probs_from_logits_response_rmpad():
    ...
    input_ids = torch.randint(
        low=0,
        high=vocab_size,
        size=(batch_size, prompt_length + response_length),
        device="cuda",
    )
    ...
    logits = torch.randn(
        batch_size, prompt_length + response_length, vocab_size, device="cuda"
    )
    ...
    assert torch.all(
        torch.eq(actual_output * response_mask, expected_output * response_mask)
    )
...
def test_lr_scheduler():
    ...
    constant_lr = get_constant_schedule_with_warmup(
        optimizer=optimizer, num_warmup_steps=2
    )
    ...
    torch.testing.assert_close(
        lr_lst, [0.0, 0.0005, 0.001, 0.0007750000000000002, 0.0003250000000000002]
    )
