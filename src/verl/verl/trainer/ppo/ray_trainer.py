...
def compute_advantage(
    data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, grpo_use_std=True
):
...
class RayPPOTrainer(object):
    ...
    def _save_checkpoint(self):
        ...
        # save sampler state
        torch.save(
            self.train_dataloader.sampler.get_state(),
            os.path.join(local_global_step_folder, "sampler_state.pt"),
            pickle_module=dill,
        )
        ...
        try:
            ...
            if self.config.trainer.max_optim_to_keep != 1:
    ...
    def _load_checkpoint(self):
        ...
        self.train_dataloader.sampler.set_state(
            torch.load(os.path.join(global_step_folder, "sampler_state.pt"))
        )
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        batch = compute_advantage(
                            ...
                            grpo_use_std=self.config.algorithm.get(
                                "grpo_use_std", True
                            ),
                        )
