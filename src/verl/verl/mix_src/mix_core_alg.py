...
def compute_token_on_off_policy_loss(
    ...
    _DBGN[0] += 1
    if _DBGN[0] <= 3:
        for _nm, _t in (("log_prob", log_prob), ("old_log_prob", old_log_prob),("ratio", ratio), ("advantages", advantages)):
            _bad = ~torch.isfinite(_t)
            if _bad.any():
                _idx = _bad.nonzero()[0].tolist()
                print("[DBG1] %s BAD n=%d nan=%d posinf=%d neginf=%d first=%s eos_there=%g prefix_there=%g" % (_nm, int(_bad.sum()), int(torch.isnan(_t).sum()),int((_t == float("inf")).sum()), int((_t == float("-inf")).sum()),_idx, float(eos_mask[tuple(_idx)]), float(prefix_mask[tuple(_idx)])), flush=True)
            else:
                print("[DBG1] %s ok absmax=%g" % (_nm, float(_t.abs().max())), flush=True)
        print("[DBG1] shape=%s eos_sum=%d prefix_sum=%d" % (tuple(log_prob.shape), int(eos_mask.sum()), int(prefix_mask.sum())), flush=True) 
    ...
    if loss_remove_clip is False:
        ...
        if _DBGN[0] <= 3:
            print("[DBG2] on_pg_losses_nf=%d on_pg_loss=%s on_mask_sum=%d" % (
                (~torch.isfinite(on_pg_losses)).sum().item(), on_pg_loss.item(),
                int(((~prefix_mask) * eos_mask).sum().item())), flush=True)
    ...
    if _DBGN[0] <= 3:
        print("[DBG4] pg_loss=%s off_pg_loss=%s on_pg_loss=%s ppo_kl=%s" % (
            pg_loss.item(), off_pg_loss.item(), on_pg_loss.item(), ppo_kl.item()), flush=True)
    return {
        "pg_loss": pg_loss,
