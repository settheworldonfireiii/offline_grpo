...
from .mix_trainer import (
    MIXRayPPOTrainer,
    apply_kl_penalty,
    compute_advantage,
    compute_data_metrics_ours,
)
...
class MIXRayPPOTrainerAccRebatch(MIXRayPPOTrainer):
        ...
        for _ in range(self.config.trainer.total_epochs):
            ...
            while True:  # batch iter
                ...
                with _timer("step", timing_raw):
                    ...
                    while (
                        len(valid_batch) < batch_size * n_samples
                    ):  # construct a valid batch
                        try:
                    ...
                    if (
                        ...
                        and self.config.actor_rollout_ref.actor.get(
                        ...
                        if self.config.actor_rollout_ref.actor.get(
                            "off_policy_normalize", True
                        ):
                            n = self.config.actor_rollout_ref.rollout.n
                            n_prefix = self.config.actor_rollout_ref.rollout.n_prefix
                            assert (
                                n_prefix != -1 and (n - n_prefix) != 1
                            )  # n-n_prefix == 1 which cause nan for std.
                            ...
                            op_logprobs = logprobs[on_policy_mask].reshape(
                                bsz, -1, l
                            )  # check whether the shape is correct
                            op_tok_logprobs = masked_mean(
                                op_logprobs, op_logprobs != 0, axis=-1
                            )  # [bsz, n_on]
                            ...
                            batch.batch["on_logprobs_mean"] = (
                                op_logprobs_mean[:, None].repeat(1, n).reshape(-1)
                            )  # [bsz]
                            batch.batch["on_logprobs_std"] = (
                                op_logprobs_std[:, None].repeat(1, n).reshape(-1)
                            )  # [bsz]
                        else:
                            ...
                            batch.batch["on_logprobs_mean"] = (
                                torch.zeros(bsz).to(dtype).to(device)
                            )
                            batch.batch["on_logprobs_std"] = (
                                torch.zeros(bsz).to(dtype).to(device)
                            )
                    ...
                    if self.config.actor_rollout_ref.actor.get(
                        ...
                        batch.batch["advantages"][prefix_mask] = (
                            reward_weight / self.config.actor_rollout_ref.rollout.n
                        )
    ...
    def add_to_buffer(self, batch, batch_size, n_samples):
        ...
        buffer_batch = batch.slice(
            range(
                batch_size * n_samples,
                (buffer_length + batch_size) * n_samples,
                n_samples,
            )
        )
        ...
        buffer_batch.slice_batch(
            start=0, length=self.config.data.max_prompt_length, dim=1
        )
    ...
    def _save_checkpoint(self):
        ...
        torch.save(
            self.train_dataloader.dataloader, dataloader_local_path, pickle_module=dill
        )
        ...
        try:
            ...
            prev_save_steps = [
                i for i in range(0, cur_step, self.config.trainer.save_freq)
            ]
            prev_save_steps = prev_save_steps[
                : -(self.config.trainer.max_optim_to_keep - 1)
            ]
            print(f"Removing optim in previous checkpoints: {prev_save_steps}")
            for each_save_step in prev_save_steps:
                prev_save_path = os.path.join(
                    self.config.trainer.default_local_dir,
                    f"global_step_{each_save_step}",
                    "actor",
                )
