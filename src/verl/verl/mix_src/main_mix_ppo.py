...
from deepscaler.rewards.math_reward import (
    THOUGHT_DELIMITER_END,
    THOUGHT_DELIMITER_START,
    deepscaler_reward_fn,
)
from verl import DataProto
from verl.mix_src.math_verify_reward import (
    reward_fn_math_verify,
    reward_fn_math_verify_no_think,
)
...
def deepscaler_reward_fn_nothink(
    ...
    def __call__(self, data: DataProto):
        ...
        def process_item(args):
            ...
            compute_score_fn = _select_rm_score_fn(
                data_source, reward_impl_version=self.reward_impl_version
            )
        ...
        if self.reward_impl_version in {3, 4}:
            ...
            with ThreadPoolExecutor(max_workers=96) as executor:
                args = [
                    (i, data[i], already_print_data_sources) for i in range(len(data))
                ]
...
def main_task(config):
    ...
    if not config.trainer.acc_rebatch:
        ...
        trainer = MIXRayPPOTrainerAccRebatch(
            config=config,
            tokenizer=tokenizer,
            role_worker_mapping=role_worker_mapping,
            resource_pool_manager=resource_pool_manager,
