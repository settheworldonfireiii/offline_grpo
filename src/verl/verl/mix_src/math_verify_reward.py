...
from deepscaler.globals import THOUGHT_DELIMITER_END, THOUGHT_DELIMITER_START
...
class RewardMathFn(RewardFn):
    ...
    def __call__(self, input: RewardInput) -> RewardOutput:
        ...
        labels = labeling_responses(
            [
                model_solution,
            ],
            input.ground_truth["answer"],
        )
        if labels[0] is True:
...
def reward_fn_math_verify_no_think(
    solution_str: str, ground_truth: Union[str, List[str]], enable_llm=False
):
    reward_config = RewardConfig()
