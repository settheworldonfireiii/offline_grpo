...
from verl.trainer.ppo.ray_trainer import (  # compute_data_metrics,; compute_advantage,
    RayPPOTrainer,
    ResourcePoolManager,
    Role,
    WorkerType,
...
def get_annealed_value(original_value, global_steps, total_steps):
    """Linear annealing from original_value to 0 over total_steps"""
    if total_steps <= 0:
        return original_value
    progress = min(global_steps / total_steps, 1.0)
    return original_value * (1.0 - progress)
...
def compute_advantage(
    data: DataProto, adv_estimator, gamma=1.0, lam=1.0, grpo_use_std=True
):
...
class MIXRayPPOTrainer(RayPPOTrainer):
    ...
    def _create_dataloader(self):
        ...
        self.train_dataset = RLHFDatasetWithTarget(
            ...
            max_num_targets=self.config.actor_rollout_ref.rollout.n_prefix,
        ...
        with open_dict(self.config):
            ...
            if self.config.trainer.save_freq == -1:
                self.config.trainer.save_freq = len(self.train_dataloader)
            if self.config.trainer.test_freq == -1:
                self.config.trainer.test_freq = len(self.train_dataloader)
    ...
    def fit(self):
        ...
        if self.config.data.get("add_tgt_with_acc", False):
            n_samples = (
                n_samples - 1
            )  # if filter tgt with acc, we either use tgt or on policy samples.
        ...
        for _ in range(self.config.trainer.total_epochs):
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    if "prefix_ratios" in gen_batch_output.meta_info.keys():
                        metrics["batch/avg_prefix_ratio"] = float(
                            np.mean(gen_batch_output.meta_info["prefix_ratios"])
                        )
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        reward_tensor = self.reward_fn(
                            batch
                        )  # [bsz, l], only the last valid token has reward

                        batch.batch["token_level_scores"] = reward_tensor

                        # Apply target rewards for off-policy samples when available
                        if (
                            "target_indices" in batch.batch
                            and "target_rewards" in batch.batch
                        ):
                            # Get the prefix mask which indicates off-policy samples
                            prefix_mask = batch.batch["prefix_mask"]
                            off_policy_mask = prefix_mask.any(-1)  # [bsz]

                            # Get indices and target rewards
                            target_indices = batch.batch["target_indices"]  # [bsz]

                            # For each off-policy sample, update the reward based on the target reward
                            for i in range(len(batch.batch["token_level_scores"])):
                                if off_policy_mask[i]:
                                    # Get the original prompt index and target index
                                    uid = batch.non_tensor_batch["uid"][i]
                                    # Find all samples with the same uid
                                    uid_indices = np.where(
                                        batch.non_tensor_batch["uid"] == uid
                                    )[0]
                                    # Find the original prompt index
                                    prompt_idx = (
                                        uid_indices[0]
                                        // self.config.actor_rollout_ref.rollout.n
                                    )

                                    # Get the target index that was used for this sample
                                    target_idx = target_indices[i].item()

                                    if target_idx >= 0:
                                        # Get the reward for this target
                                        target_reward = batch.batch["target_rewards"][
                                            prompt_idx, target_idx
                                        ].item()

                                        # Set the reward at the final token position
                                        response_mask = batch.batch["attention_mask"][
                                            i, -batch.batch["responses"].size(1) :
                                        ].bool()
                                        valid_response_length = (
                                            response_mask.sum().item()
                                        )

                                        # Set the success/failure value based on target_reward
                                        if self.config.data.reward_impl_version in (
                                            0,
                                            2,
                                            3,
                                            4,
                                            ...
                                            value = 1.0 if target_reward > 0.5 else 0.0
                                        elif self.config.data.reward_impl_version == 1:
                                            # Range from -0.5 to 1.0
                                            value = 1.0 if target_reward > 0.5 else -0.5
                                        else:
                                            value = target_reward

                                        # Apply the reward to the final token
                                        batch.batch["token_level_scores"][
                                            i, valid_response_length - 1
                                        ] = value
                        ...
                        if self.config.data.reward_impl_version == 0:
                            ...
                            raise ValueError(
                                f"Invalid reward implementation version: {self.config.data.reward_impl_version}"
                            )
                        ...
                        # add more metrics
                        metrics["batch/solved"] = (
                            reward_tensor.sum(-1) == success_value
                        ).sum().item() / len(uids)
                        metrics["batch/failed"] = (
                            reward_tensor.sum(-1) == fail_value
                        ).sum().item() / len(uids)
                        ...
                        metrics["batch/on_solved"] = (
                            reward_tensor[on_policy_mask].sum(-1) == success_value
                        ).sum().item() / (on_policy_mask.sum().item() + 1e-6)
                        metrics["batch/off_solved"] = (
                            reward_tensor[off_policy_mask].sum(-1) == success_value
                        ).sum().item() / (off_policy_mask.sum().item() + 1e-6)
                        ...
                        # Apply add_all_positive: Set advantages to 0.5 when all samples in group are positive
                        if (
                            hasattr(self.config.algorithm, "add_all_positive")
                            and self.config.algorithm.add_all_positive
                        ):
                            ...
                            # Calculate total reward per sample: sum over response length dimension
                            total_rewards = (token_level_rewards * non_zero_mask).sum(
                                dim=-1
                            )

                            # Group by uid and check if all samples in group are positive
                            uids = batch.non_tensor_batch["uid"]
                            unique_uids = np.unique(uids)
                            ...
                            for uid in unique_uids:
                                # Find all samples with the same uid (same question)
                                uid_mask = uids == uid
                                uid_indices = np.where(uid_mask)[0]
                                group_rewards = total_rewards[uid_indices]

                                # Check if all samples in this group are positive (> 0)
                                all_positive = (group_rewards > 0).all()

                                if all_positive:
                                    # Set advantages to bias_value for all samples in this group
                                    for idx in uid_indices:
                                        advantages[idx] = (
                                            advantages[idx] * (1 - response_mask[idx])
                                            + bias_value * response_mask[idx]
                                        )
                        ...
                        # For multi-target mode, ensure target_indices are passed to the actor
                        if "target_indices" in batch.batch:
                            metrics["batch/has_target_indices"] = 1
                        else:
                            metrics["batch/has_target_indices"] = 0

                        # For multi-target mode, ensure target_probs_lst is passed to the actor
                        if "target_probs_lst" in batch.batch:
                            metrics["batch/has_target_probs_lst"] = 1
                        else:
                            metrics["batch/has_target_probs_lst"] = 0

                        alpha_weight = (
                            prefix_mask.float()
                            * self.config.actor_rollout_ref.rollout.prefix_reward_weight_alpha
                        )
                        beta_weight = (
                            ~prefix_mask
                        ).float() * self.config.actor_rollout_ref.rollout.prefix_reward_weight_beta
                ...
                # collect metrics
                metrics.update(
                    compute_data_metrics_ours(batch=batch, use_critic=self.use_critic)
                )
                metrics.update(
    ...
    def maybe_save_best_hf(self, val_metrics: dict):
        ...
        actor_local_path = os.path.join(
            self.config.trainer.default_local_dir, "best", f"actor"
        )
        ...
        if cur_score > best_score:
            print(
                f"Saving best checkpoint with score {cur_score} at {actor_local_path}"
            )
            ...
            with open(f"{actor_local_path}/metrics.json", "w") as f:
                f.write(
                    json.dumps(
                        {"best_avg_score": best_score, "global_step": self.global_steps}
                    )
                    + "\n"
                )
...
def compute_data_metrics_ours(batch, use_critic=True):
    ...
    if on_policy_mask.sum().item() != 0:
        off_on_example_ratio = (
            off_policy_mask.sum().item() / on_policy_mask.sum().item()
        )
    else:
        off_on_example_ratio = -1
    ...
    metrics = {
        ...
        **(
        ...
        "on_off_metrics/on_response_length_mean": torch.mean(on_response_length)
        .detach()
        .item(),
        "on_off_metrics/off_response_length_mean": torch.mean(off_response_length)
        .detach()
        .item(),
        "on_off_metrics/on_score": torch.mean(on_sequence_score).detach().item(),
        "on_off_metrics/off_score": torch.mean(off_sequence_score).detach().item(),
        ...
        "on_off_metrics/off_on_example_ratio": off_on_example_ratio,
        # prompt length
